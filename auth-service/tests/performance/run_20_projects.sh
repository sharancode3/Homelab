...
echo "=== PHASE 20 20-PROJECT LOAD STAGE ==="

echo "Starting monitor in background..."
.venv/bin/python tests/performance/monitor.py 70 > tests/performance/monitor_20.log 2>&1 &
MONITOR_PID=$!

echo "Starting load test for 20 projects..."
.venv/bin/python tests/performance/load_test.py 20 > tests/performance/20_project_report.txt 2>&1
...
REPORT="tests/performance/20_project_metrics.txt"
date > $REPORT
