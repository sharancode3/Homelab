...
echo "=== PHASE 20 50-PROJECT LOAD STAGE ==="

echo "Starting monitor in background..."
.venv/bin/python tests/performance/monitor.py 70 > tests/performance/monitor_50.log 2>&1 &
MONITOR_PID=$!

echo "Starting load test for 50 projects..."
.venv/bin/python tests/performance/load_test.py 50 > tests/performance/50_project_report.txt 2>&1
...
REPORT="tests/performance/50_project_metrics.txt"
date > $REPORT
